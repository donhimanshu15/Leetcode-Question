        ...
        for(int i=0; i<board.size(); i++)
        {
            for(int j=0; j<board[0].size(); j++)
            {
                if(board[i][j]=='.')
                {
                    ...
                    return false; // signifies that we were not able to put any character at that position. This may happen in case we had filled the wrong one out of the multiple options for a position in the previous call. And, now based on that, no character can be filled here because its violating either of the 3 rules to be valid.// 
                }
            }
        }
